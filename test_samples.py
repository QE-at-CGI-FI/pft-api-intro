...
def test_challenge_02():
    ...
    #This approval helps us track our progress - it fails again when we make progress through tasks
...
def test_challenge_08():
    headers = {"X-CHALLENGER": MY_TRACKING_CODE}
    r = requests.head(TODOS_URL, headers=headers)
    assert r.status_code == 200
...
def test_challenge_09():
    headers = {"X-CHALLENGER": MY_TRACKING_CODE}
    todo_data = {"title": "New Todo", "description": "Testing 123", "doneStatus": False}
    r = requests.post(TODOS_URL, headers=headers, json=todo_data)
    assert r.status_code == 201
    approvaltests.verify_as_json(r.json())
...
def test_challenge_10():
    headers = {"X-CHALLENGER": MY_TRACKING_CODE}
    todo_data = {"title": "New Todo", "description": "Testing 123", "doneStatus": "not_a_boolean"}
    r = requests.post(TODOS_URL, headers=headers, json=todo_data)
...
def test_challenge_11():
    headers = {"X-CHALLENGER": MY_TRACKING_CODE}
    todo_data = {"title": "New Todo" * 100, "description": "Testing 123", "doneStatus": False}
    r = requests.post(TODOS_URL, headers=headers, json=todo_data)
    assert r.status_code == 400
    approvaltests.verify_as_json(r.json())
...
def test_challenge_12():
    headers = {"X-CHALLENGER": MY_TRACKING_CODE}
    todo_data = {"title": "New Todo", "description": "Testing 123" * 100, "doneStatus": False}
    r = requests.post(TODOS_URL, headers=headers, json=todo_data)
    assert r.status_code == 400
    approvaltests.verify_as_json(r.json())
...
def test_challenge_15():
    headers = {"X-CHALLENGER": MY_TRACKING_CODE}
    todo_data = {"title": "New Todo", "description": "Testing 123", "doneStatus": False, "extraField": "not allowed"}
    r = requests.post(TODOS_URL, headers=headers, json=todo_data)
    assert r.status_code == 400
    approvaltests.verify_as_json(r.json())
